...
func Migrate(driverName, dataSource, dbName string) error {
	db, err := sql.Open(driverName, dataSource)
	if err != nil {
		return err
	}
...
func truncPhoneNumbersTable(db *sql.DB) error {
	_, err := db.Exec("TRUNCATE ONLY phone_numbers RESTART IDENTITY")
	return err
}
